# Solve the social-optimum fee path: platforms set fees to maximise total
# welfare (consumer surplus + restaurant profits + platform profits), rather
# than competing in Bertrand-Nash for own profit. Output:
# output/CF_feefirst/spec<nsim>/socopt_<code>.rds, one per market.
# Consumed by decompose_rpi_soc_fees.R, compare_priv_soc.R, analyze_max_joint.R,
# variety_vs_fixed_costs_soc_priv.R.
#
# Warm-starts from previously-solved baseline_*.rds equilibria.
library(Matrix)
...
main <- function(){
    ...
    init.files <- grep('^baseline_[a-z]+\\.rds$', list.files(cf.dir), value = TRUE)
    for (f in init.files){
    ...
    ncores <- as.integer(Sys.getenv('CF_NCORES', 8))

    ncores <- min(length(markets), ncores)
