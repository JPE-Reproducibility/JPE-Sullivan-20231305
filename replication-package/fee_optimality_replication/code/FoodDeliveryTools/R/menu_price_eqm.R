menu.price.eqm <- function(dat.m, mc, num.param,
    ...
    if (!('phi' %in% names(dat.m))){
        stop("dat.m$phi (the commission-accounting parameter vartheta) is missing; ",
             "load it from the restaurant-cost estimates (rMC$phi) before computing a pricing equilibrium")
    }
    phi <- dat.m$phi
    ...
    ## values used in computing initial price
    ratio <- 0.10
    pdiff <- 3
        ...
        for (g in 1:nportfolios){
            ...
            if (g %in% keep.portfolios){
                ...
                if (ng == 1){
                    ...
                    init.rho <- c(mc.off.z, rep(mc.on.z, times = ng - 1))
                    init.rho <- init.rho/(1 - phi*R.guess[idx.g])
                    init.rho <- init.rho + b
    ...
    for (iter in 1:max.iter){
        ...
        for (zip in menu.subset){
            ...
            for (g in 1:nportfolios){
                ...
                s.tilde <- s.0*matrix(1 - phi*R.zg, ncol = 1)
                ## Scale down s.tilde if there is less than one restaurant
                ...
                dist.z.g <-  tryCatch({
                    ...
                    if (pm.in){
                        if (pm.small){
                            ...
                            mkp <- rho.pm*(1 - phi*R.zg[pm.idx]) - mc.g[pm.idx]
        ...
        if (is.nan(dist)){
            ...
            warning(sprintf(paste0('Restaurant price algorithm did not converge ',
