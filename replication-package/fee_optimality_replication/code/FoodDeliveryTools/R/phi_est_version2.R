phi.objective.v2 <- function(phi, dat, rhos, opts, b = NULL){
    ...
    for (market in markets){
        dat.m <- dat[[market]]
        costs[[market]] <- compute.costs.phi.v2(dat.m, rhos, phi, opts)
    }
    ...
    if (is.null(b)){
        ...
        fval <- mean(Costs.1.df$omega[idx]*Costs.1.df$IV[idx])
    }
...
compute.costs.phi.v2 <- function(dat.m, rhos, phi, opts){
    # phi is the parameter that governs internalization of the
    # commission rate
    ...
    for (zip in zips){
        ...
        for (g in 1:nportfolios){
            ...
            s.tilde <- s.0*matrix(1 - phi*R, ncol = 1)
            ...
            MC.zg <- Rhos.0[[zip]][[g]]*(1 - phi*R) - b
            # Store objects
            markups[[zip]][[g]] <- b
            costs[[zip]][[g]]   <- MC.zg
            # Store weights
            weights[[zip]][[g]] <- pracma::repmat(matrix(J.G.1.m[[zip]][g], ncol = 1),
                                                  length(idx.g), 1)
        }
    }
